...
class AttentionRegularizer(regularizers.Regularizer):
    # properties
    _r: float
    _alpha: float
    ...
    def __call__(self, attentions: List[tf.Variable]) -> float:
        ...
        pruning_ratio = tf.convert_to_tensor(attentions)
        pruning_ratio = tf.clip_by_value(pruning_ratio, 0.01, 1)
        pruning_ratio = tf.pow(1 - pruning_ratio, self._alpha) # type: ignore
        pruning_ratio = tf.clip_by_value(pruning_ratio, 0, 0.99)

        # S = (1 - p) * nW / sum(nW)
        penalty = (1 - pruning_ratio) * tf.convert_to_tensor(self.filters, dtype=tf.float32) # type: ignore
        penalty = tf.reduce_sum(penalty) / tf.cast(tf.reduce_sum(self.filters), tf.float32)

        # penalty = decay * S^2
        penalty -= self._r
        penalty = tf.math.square(penalty)
        penalty = self._attention_decay * penalty # type: ignore
        return penalty
